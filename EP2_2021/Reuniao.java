...
public class Reuniao {
  ...
  void defineInterseccoes(int index, LocalDateTime inicio, LocalDateTime fim){
    ...
    for(Disponibilidade inicioAtual : listaIntervalos){
      ...
      if(inicioAtual.getInicio().isBefore(fim) && inicioAtual.getFim().isAfter(inicio)){
        if(inicioAtual.getFim().isBefore(fim)) fim = inicioAtual.getFim();
        if(inicioAtual.getInicio().isAfter(inicio)) inicio = inicioAtual.getInicio();
    ...
    for(Participante participante : participantes){
      ...
      if(listaDisponibilidade != null) {
        for(Disponibilidade disponibilidade : listaDisponibilidade){
          System.out.println("De " + disponibilidade.getInicio().format(formatter) + " até " + disponibilidade.getFim().format(formatter));
        }
      }
      System.out.println("");
    }
